...
if(isset($_POST['name']) && !empty($_POST['name']) && !empty($_POST['vehicle']) ){
    ...
    if($udata['balance']>=$price['price']){
        ...
        // printR($_FILES);
        if($type == "image/jpeg"  && $type2 == "image/jpeg"){
        ...
        $upload = "uploads/";
        if(move_uploaded_file($_FILES['front_side']['tmp_name'],$upload.$ext) && move_uploaded_file($_FILES['back_side']['tmp_name'],$upload.$ext2)){
            $fee = $price['price'];
            ...
            $submit = mysqli_query($ahk_conn,"INSERT INTO `2wheelerpuc` (`appliedby`, `name`, `vehicle`, `status`, `fee`,`date`,`front_side`,`back_side`) VALUES('$appliedby', '$name','$vehicle','pending','$fee','$date','$ext','$ext2')");
            if($submit == true){
